  ...
  return (
    <div className="bg-navy py-4 text-white md:py-5">
      <div className="xsto-container">
        <div className="mb-3 grid grid-cols-2 gap-x-5 gap-y-4 md:mb-3.5 md:grid-cols-12 md:gap-x-6 lg:gap-x-8">
          <div className="col-span-2 space-y-2 md:col-span-4">
            <img
              alt={FOOTER_LOGO.alt}
              className="h-12 w-auto max-w-[min(100%,13rem)] rounded-none object-contain object-left sm:h-14 sm:max-w-[15rem]"
              decoding="async"
            ...
            <address className="space-y-0 not-italic text-sm leading-snug text-white/85">
              <p>
                <a
...
function FooterLinkColumn({
  ...
  return (
    <nav aria-label={title} className={className}>
      ...
      <ul className="grid grid-cols-1 gap-x-5 lg:grid-cols-2">
        {links.map((link) => (
          <li key={link.url}>
            <NavLink
              className="flex min-h-8 items-center py-1 text-[0.8125rem] leading-tight text-white/85 transition-colors hover:text-white sm:text-sm"
              prefetch="intent"
              to={link.url}
            >
              {link.title}
...
function SocialLink({
  ...
  return (
    <a
      aria-label={label}
      className="inline-flex size-8 items-center justify-center rounded-full border border-white/25 text-white transition-colors hover:border-white/50 hover:bg-white/10"
      href={href}
